...
def run_single(system_prompt, task_prompt, run_dir, run_idx):
    """단일 벤치마크 실행 (claude -p 사용)"""
    ...
    try:
        result = subprocess.run(
            ["claude", "-p", task_prompt, "--system-prompt", system_prompt,
             "--output-format", "json"],
        ...
        print("[ERROR] 'claude' CLI not found. Install Claude Code CLI first.", file=sys.stderr)
        run_result = {
            "run_idx": run_idx,
            "success": False,
            "exit_code": -2,
...
def run_benchmark(osss_dir, task_prompt, n_runs=10, task_class=None):
    """전체 벤치마크 실행"""
    ...
    for ci, cand in enumerate(candidates):
        ...
        for ri in range(n_runs):
            r = run_single(prompt, task_prompt, cand_dir, ri)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: benchmark.py run --task-prompt <prompt> [--runs N] [--task-class <class>]")
    ...
    if cmd == "run":
        ...
        run_benchmark(osss_dir, task_prompt, n_runs, task_class)
