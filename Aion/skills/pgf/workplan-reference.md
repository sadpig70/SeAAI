...
Loop Skill is the most complete expression of PGF's parser-free property: **the execution engine itself is specified in PGF**. Antigravity Agent reads loop-reference.md to understand the execution algorithm, then applies it to WORKPLAN-{Name}.md.
...
loop-reference.md    →  Execution engine (itself a PGF document)
     ↓ runtime
Antigravity Agent            →  AI runtime (runtime, not a tool)
```
...
Steps performed by Antigravity Agent:
