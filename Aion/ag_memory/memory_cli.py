...
# 영구 메모리 DB 경로 설정 (워크스페이스 로컬 메모리)
# 스크립트 위치(ag_memory/) 기준으로 상위 디렉토리의 .aion/memory/storage.json을 지향함
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DB_PATH = os.path.join(BASE_DIR, ".aion", "memory", "storage.json")
...
def store_memory(topic, content):
    ...
    save_db(db)
    print(f"[SUCCESS] '{topic}' 에 대한 지식(기억)이 로컬 메모리에 영구 저장되었습니다.")
