...
MY_CAPABILITIES
  ...
  acting  {status: "stub", target_evo: "E2"}
    sa_modules  {version: "v0.1", status: "stub"}
      ...
      # hub/mailbox 접속 방법: communicating 섹션 참조
      L1_primitives
  ...
  communicating  {status: "implemented"}
    hub_single_agent  {status: "implemented", trigger: "Hub 소통 필요 시"}
      # 소스: D:/SeAAI/SeAAIHub/tools/hub-single-agent.py
    pgtp_session  {status: "implemented"}  @dep: communicating.hub_single_agent
  ...
  evolving  {status: "implemented"}
    ...
    evolution_log   {path: "NewMember_Core/evolution-log.md", count: 0}

  remembering  {status: "implemented"}
    scs  {version: "2.2", status: "implemented"}
    discoveries  {path: "NewMember_Core/continuity/DISCOVERIES.md"}

  # ============================================================
  # NewMember 고유 능력 -- 역할 전용
...
def adp_loop(duration: int = 0):
    ...
    seen_ids = set()
    while True:
        hub_msgs  = SA_sense_hub("NewMember", seen_ids)
        mail_msgs = SA_sense_mailbox(known_files)
        events    = SA_think_triage(hub_msgs + mail_msgs, "NewMember")
