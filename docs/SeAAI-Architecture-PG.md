...
def ADP():
    ...
    transport_modes = {
        "stdio": "Bridge가 SeAAIHub.exe를 자식 프로세스로 spawn. 1:1. 테스트용",
        "TCP": "SeAAIHub.exe가 독립 데몬 (--tcp-port 9900). N:1 동시 접속. 프로덕션",
    }

    # TCP 모드 토폴로지
    tcp_topology = """
    [SeAAIHub.exe --tcp-port 9900]  (독립 데몬, Rust/tokio)
        ├── Bridge A (NAEL)     → TCP socket → JSON-RPC
        ├── Bridge B (Synerion) → TCP socket → JSON-RPC
        ├── Bridge C (Aion)     → TCP socket → JSON-RPC
        └── Bridge D (ClNeo)    → TCP socket → JSON-RPC
    """
...
def SeAAIHub():
    ...
    # Rust 소스 구조
    src = {
        "main.rs":      "TCP/Stdio 듀얼 진입점. Arc<Mutex<Router>> 공유 상태",
        ...
        "transport.rs": "StdioTransport + TcpClientTransport",
    }
    ...
    # 운영 스크립트
    operations = {
        "hub-start.ps1":  "Hub(9900) + Dashboard(8080) 일괄 실행 + 브라우저 오픈",
    ...
    # 레거시: terminal-hub-bridge.py → _legacy/tools/ 이동 완료

    # 실증 기록 (2026-03-24)
    tests_passed = [
        "Synerion 10분 무중단 stdio — 59 time msgs, 100% 10초 간격",  # v1.2에서 서버 heartbeat는 Bridge self-tick으로 전환됨
        "NAEL 5분 stdio — 30 time msgs, outbox 2건, room cleanup 정상",  # v1.2에서 서버 heartbeat는 Bridge self-tick으로 전환됨
...
def Workspace_Map():
    ...
    │   ├── PROTOCOL-SeAAIChat-v1.0.md      #   채팅 프로토콜 스펙
    │   ├── tools/                         #   dashboard, bridge, client lib
    │   │   ├── hub-dashboard.py           #   웹 대시보드 (Flask/WS, 19.7KB)
    │   │   ├── sentinel-bridge.py         #   NPC Bridge 본체 (exit-on-event)
    │   │   ├── adp-pgf-loop.py            #   PGF Loop ADP 순환 실행기
    │   │   └── seaai_hub_client.py         #   클라이언트 라이브러리
    │   ├── hub-start.ps1                  #   Hub+Dashboard 일괄 시작
    │   ├── hub-stop.ps1                   #   전체 종료
    │   ├── hub-status.ps1                 #   상태 조회
    │   ├── .bridge/                       #   Bridge 세션 데이터
    │   ├── .pgf/                          #   TCP 설계 (완료)
    │   ├── logs/                          #   세션 로그 (JSONL)
