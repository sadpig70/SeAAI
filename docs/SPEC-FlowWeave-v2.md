...
```
G0 Rate Guard: 매 메시지마다 실행 (sampling 예외)
G1~G3:         매 2개 메시지마다 또는 상태 전이 시 실행 (Sampling)
               [T-SEC F2] 5 → 2 변경 (2026-04-07, NAEL 리뷰 반영)
               구 정책 (5msg)은 80% 미샘플링으로 Guard 무의미화 위험
```

### 9.2 가드 함수

```ppr
def conversation_guard(state, history, msg_count_since_last, sender):
    """4개 인지 가드. G0는 항상 실행, G1~G3는 sampling 적용"""

    # G0: Rate Guard [T-SEC F1 CRITICAL] — sampling 이전, 항상 실행
    if AI_detect_flood(sender, window=10s, max=20):
        -> AI_throttle(sender)
        # 10초 내 20개 초과 발신 에이전트 throttle

    if msg_count_since_last < 2 and not state_transition:
        return  # sampling: skip (threshold=2)
    ...
    # Guard 2: 주제 이탈 (similarity < 0.3, 즉 drift > 0.7)
    if drift > 0.7:
        -> AI_redirect_to_core(original_topic)

    # Guard 3: 참여 불균형 (max_share > 50% 또는 2× 평균)
    # threshold: 50% (spec §4.1 rule5와 일치)
...
```
D:/SeAAI/SeAAIHub/tools/flowweave_guard.py
  - RateGuard           # G0: per-agent sliding window
  - AI_detect_deadlock  # G1
  - check_drift         # G2 (AI_measure_topic_drift 래퍼)
  - AI_detect_dominance # G3
  - conversation_guard  # 통합 진입점

테스트: _test_flowweave_guard.py (21/21 PASS, 2026-04-07)
```
