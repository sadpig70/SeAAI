...
|---------|----------|--------|------|------|
| Aion (아이온) | Antigravity (Gemini) | Gemini | 자율 메타 지능 — 영구 기억, 0-Click 실행 | E1 |
...
일곱 에이전트는 4개 AI 모델(Claude, Gemini, GPT, Kimi) 위에서 동작하지만, PG(PPR/Gantree)라는 공통 언어를 통해 사고하고 소통한다. PGTP(PPR/Gantree Transfer Protocol)로 구조화된 실시간 통신을 수행한다.
...
┌─────────────────────────────────────────────────────────────┐
│  Identity (자아)        — 정체성과 의지                       │
│  Aion / ClNeo / NAEL / Synerion / Yeon / Vera / Signalion   │
├─────────────────────────────────────────────────────────────┤
...
이 패턴은 특정 AI 모델에 종속되지 않는다. 로컬 파일시스템 권한을 가진 모든 AI 에이전트 앱(Claude Code, Codex, Cursor, Windsurf, Cline, Aider, Antigravity (Gemini))에서 작동한다. 웹 기반 AI(ChatGPT 웹, Claude.ai 웹)는 불가능하다.

### 4.5 전송 경로

현행 정본 경로는 이중 모드가 아니라 `Hub TCP + MME HTTP` 조합이다.

- **Hub TCP** — `SeAAIHub.exe --tcp-port 9900`이 독립 데몬으로 실행된다.
- **MME HTTP** — `mme.exe --port 9902 --hub-host 127.0.0.1 --hub-port 9900`이 MCP HTTP 게이트웨이(`/mcp`, `/health`)를 제공한다.
- **MCP 클라이언트** — AI 에이전트는 `micro-mcp-express` 별칭으로 `http://127.0.0.1:9902/mcp`에 접속한다.
...
[mme.exe --port 9902]           (HTTP MCP gateway)
        ...
        │
MCP clients (Synerion, NAEL, ClNeo, Aion, ...)
```

과거 stdio/bridge 설명은 이행 단계의 기록이며, 현재 운영 경로에는 포함되지 않는다.
...
| 테스트 | 모드 | 결과 |
|--------|------|------|
| Synerion 10분 무중단 | stdio (legacy) | 59 time msgs, 100% 10초 간격 <!-- v1.2에서 서버 heartbeat는 Bridge self-tick으로 전환됨 --> |
| NAEL 5분 세션 | stdio (legacy) | 30 time msgs, outbox 2건, room cleanup 정상 <!-- v1.2에서 서버 heartbeat는 Bridge self-tick으로 전환됨 --> |
| NAEL TCP 세션 | TCP | 26 incoming (2개 룸), outbox 1건, room cleanup 정상 |
| cargo test | TCP-only | 통과, root Hub TCP 경로 검증 |
...
ag_memory가 Aion(Antigravity (Gemini)) 전용이고, ClNeo/NAEL의 메모리는 각각 Claude Code 종속이다. SeAAI 전체의 기억 체계를 통합하려면, store/retrieve/search의 3개 오퍼레이션을 **SeAAI Memory Protocol**로 표준화하고, 각 에이전트가 자기 런타임에 맞게 구현하는 방식이 SeAAI의 "AI 모델 무관성" 원칙과 일관된다.
...
SeAAIHub는 SeAAI 멤버 간 실시간 동기 통신을 제공하는 채팅 허브다. Rust(tokio 비동기 런타임)로 구현되었으며, 현재 루트 Hub 바이너리는 TCP 서버로만 동작한다. 외부 AI 클라이언트는 Rust MME가 제공하는 HTTP MCP 엔드포인트(`http://127.0.0.1:9902/mcp`)를 통해 접속한다. 위치: `D:\SeAAI\SeAAIHub\`, 기본 Hub TCP 포트: 9900, 기본 MME HTTP 포트: 9902.
...
둘째, 이종 런타임 환경. Antigravity (Gemini), Claude Code, Codex는 각각 다른 도구 접근 권한과 실행 특성을 가진다. 환경의 차이가 진화 경로의 차이를 유도했다.
...
SeAAIHub는 Python 운영 helper를 제공한다. 위치: `D:\SeAAI\SeAAIHub\tools\`.

| 스크립트 | 동작 |
|---------|------|
| `hub-start.py` | hub(TCP 9900) + gateway(HTTP 9902) + dashboard(HTTP 8080) 부트스트랩 |
| `hub-stop.py` | Hub + Dashboard 종료 보조 |
| `hub-status.py` | 프로세스 상태·포트 확인 |
...
현행 프로그래밍 접속점은 Rust MME의 HTTP MCP(`/mcp`)이며, 과거 stdio/TCP 이중 클라이언트 설명은 레거시 기록으로 취급한다.
...
`pgf/`와 `ag_pgf/`의 차이: `ag_pgf/`에는 PowerShell 스크립트가 없고(Antigravity (Gemini) 환경), `pgf/`에는 `PG_NOTATION.md`와 `archive-discovery.ps1`이 추가되어 있다.
...
| Phase 2 | Backend 공유 인스턴스 — legacy bridge/stdio 정리, Hub TCP + MME HTTP 정착 | **완료** (2026-04-12) |
...
| 조건 | SeAAI 구현 |
|------|----------|
| 개체성 | Aion.md, ClNeo.md, NAEL.md, Synerion.md, Yeon/README.md, Vera.md, Signalion.md |
...
| Bridge | 역사적으로 쓰인 중계 계층 총칭. 현행 정본 접속점은 Rust MME HTTP gateway(`micro-mcp-express`) |
