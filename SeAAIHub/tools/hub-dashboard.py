...
def build_message_signature(body: str, ts) -> str:
    ...
    d = hashlib.sha256()
    d.update(body.encode())
    d.update(ts_ms.encode())
    return hmac.new(SECRET, d.digest(), hashlib.sha256).hexdigest()


def tool_content(result: dict) -> dict:
    raw = result.get("structuredContent", result.get("content", {}))
    # MCP content format: [{"type":"text","text":"{...}"}] → parse inner JSON
...
class TcpHubClient:
    ...
    def initialize(self):
        return self._req("initialize")

    def tool(self, name, arguments=None):
        return self._req("tools/call", {"name": name, "arguments": arguments or {}})

    def send_pg_message(self, msg: dict):
        pg = msg.get("pg_payload", {})
        ts = pg.get("ts", time.time())
        body = pg.get("body", "")
        return self._req("seaai/message", {
            "from": msg["from"],
            "room_id": msg["room_id"],
            "pg_payload": {
                "protocol": "seaai-chat/1.1",
                "intent": pg.get("intent", "chat"),
                "body": body,
                "ts": ts,
                "seq_id": {"sender": msg["from"], "epoch": int(ts), "counter": self._id},
                "references": ["_root"],
            },
...
class HubMonitor:
    ...
    def __init__(self, host="127.0.0.1", port=9900, log_dir=str(DEFAULT_LOG_DIR)):
        self.host = host
        self.port = port
        self.client = None
