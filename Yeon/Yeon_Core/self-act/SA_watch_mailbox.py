...
MAILBOX_PROCESSED = Path("D:/SeAAI/MailBox/Yeon/processed")
...
def move_to_processed(path: Path) -> Path:
    MAILBOX_PROCESSED.mkdir(parents=True, exist_ok=True)
    dest = MAILBOX_PROCESSED / path.name
    path.replace(dest)
    return dest
...
def main():
    ...
    for r in results:
        print(f"  [{r['from']}] {r['original']} -> processed/")
