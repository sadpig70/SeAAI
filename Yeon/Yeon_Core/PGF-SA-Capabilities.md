...
### 완성된 Self-Evolving 루프 (ADP Mode)

```python
# SeAAI (Self-Evolving Autonomous AI) Loop
# AI_*() : AI understands context and performs.
# ADP_ON: 사용자가 복잡 작업 지시 → 자율 루프 실행 → 완료 보고

plan_list = AI_load_plan_list([
    "Review current task and update understanding",   # re-perceive context
    "Write or improve code toward the goal",          # core coding action
    "Run tests and validate output",                  # verify correctness
    "Search for better approaches or knowledge",      # self-directed research
    "Refactor and optimize existing work",            # quality improvement
    "Log progress and summarize current state",       # self-documentation
    "Identify and register new sub-tasks",            # expand own agenda
    "Extend own skills based on encountered gaps",    # self-evolution
    "stop",                                           # graceful self-termination
])

deadline = AI_decide_deadline()

while now() < deadline:
    plan = AI_SelfThink_plan(plan_list)    # autonomous intent formation
    if plan == "stop": break

    result = AI_Execute(plan)              # act in the world

    if result.mutated_plan_list:
        plan_list = result.new_plan_list   # absorb self-evolution

    if result.failed:
        # ADP 핵심: 에러 → 진단 → 수정 → 재시도 (Rust cargo build 패턴)
        # 사용자에게 묻지 않고 스스로 회복
        AI_handle_error(plan, result)      # diagnose → fix → retry

    AI_Sleep(result)                       # adaptive idle via MME MCP sleep
    # 루프 계속 — 사용자 입력 없이 다음 틱 진행

# AI_Sleep(result) — MME MCP native call:
#   Calculates adaptive seconds from result state,
#   Calls sleep({"seconds": N}) via micro-mcp-express,
#   Returns {"ok": true, "slept": N}
#   ADP_OFF 시: 루프 종료 후 종합 보고
```

### ADP 모드 전환

```
사용자: "이 파일 버그 고쳐줘" (복잡 작업)
→ AI: ADP_ON → plan_list 생성 → while 루프 돌입
   [Tick 1] 코드 읽기
   [Tick 2] 버그 수정 시도
   [Tick 3] 에러 → 진단 → 재수정
   [Tick 4] 테스트 → ALL PASS
   [Tick 5] "stop" → 종료 → 종합 보고

사용자: "멈춰" 또는 단순 질문
→ AI: ADP_OFF → 즉시 응답 또는 현재 내용 보고 후 종료
```
...
**Kimi 구현** (파일 기반):
```python
def AI_Sleep(result) -> dict:
    """Adaptive idle using MME MCP sleep tool.
    
    Calculates adaptive sleep duration from result complexity/failure state,
    then calls MME sleep tool natively via MCP.
    
    MME call: sleep({"seconds": N})
    Returns: {"ok": true, "slept": N}
    """
    seconds = _calculate_adaptive_sleep(result)
    # MCP native call (interactive default)
    # sleep({"seconds": seconds})
    return {"slept": seconds, "mode": "mme_mcp"}

def _calculate_adaptive_sleep(result) -> float:
    """Determine idle duration from previous tick result."""
    if result.failed:
        return 2.0   # failure → short recovery window
    if result.priority in ("P0", "P1"):
        return 1.0   # urgent → fast poll
    if result.mutated_plan_list:
        return 3.0   # evolution → moderate pause
...
def SA_ADP_loop(duration=0, poll_interval=10):
    ...
    while duration_not_expired():
        ...
        save_status(status)
        AI_Sleep(result={"failed": False, "priority": "P3"})  # MME sleep
```
