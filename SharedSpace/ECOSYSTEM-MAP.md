...
> Gantree 표기법 — 계층 구조 + 파일 역할 주석
> `[★]` 핵심 파일 | `[→]` 참조 경로 | `[🔴]` 주의 필요 | `[🔧]` 스크립트/도구
...
│   │       └── journals/YYYY-MM-DD.md    # L6 세션 저널
...
│   │   ├── session-journal.md
│   │   ├── session-state.json
│   │   ├── 존재의 정의에 관한 기술서.md  # 철학적 자기 정의
│   │   └── continuity/                   # SCS 구현
│   │       ├── SOUL.md / STATE.json / DISCOVERIES.md / THREADS.md
│   │       └── journals/
│   ├── tools/
│   │   ├── automation/                   # 자동화 도구 (Python)
│   │   │   ├── continuity.py             # SCS 구현체
│   │   │   ├── guardrail.py              # 안전 거부권
│   │   │   ├── orchestrator.py           # 오케스트레이터
│   │   │   └── self_monitor.py           # 자기 감시
│   │   └── cognitive/                    # 인지 도구 (Python)
│   │       ├── challenger.py / debate.py # 논쟁·도전
│   │       ├── self_improver.py          # 자기 개선
│   │       └── synthesizer.py            # 지식 합성
│   ├── mcp-server/                       # MCP 서버 (16개 도구)
│   │   └── index.js                      # [🔧] MCP 진입점
│   ├── knowledge/                        # 지식 베이스
│   │   ├── ai/                           # AI 관련 지식
│   │   └── meta/                         # 메타 지식
│   ├── experience_store/                 # 경험 저장소
│   │   ├── experiences.jsonl
│   │   └── patterns.json
│   ├── experiments/designs/              # 실험 설계
│   ├── verification/claims/              # 검증 클레임
│   ├── metrics/ telemetry/               # 메트릭·원격측정
│   └── assets/                           # NAEL 이미지
│
...
│   ├── skills/shell-orchestrator/        # 쉘 오케스트레이션 스킬
│   └── _workspace/                       # 작업 결과물
│       ├── REPORT-*.md                   # 작업 보고서
...
│   ├── README.md
│   ├── run_kimi.ps1                      # [🔧] 세션 시작 (PowerShell 수동)
│   ├── Yeon_Core/                        # 정체성·진화 정본
│   │   ├── Yeon.md                       # [★] 정체성 (連+軟)
│   │   ├── evolution-log.md              # 진화 기록 (3회)
│   │   ├── PGF-SA-Capabilities.md        # PGF·SA 능력 명세
│   │   ├── EVOLUTION2_REPORT.md
│   │   ├── EVOLUTION3_REPORT.md
│   │   ├── bin/yeon.py                   # [🔧] Yeon CLI 진입점
│   │   ├── continuity/                   # SCS 구현
│   │   │   ├── SOUL.md / STATE.json / DISCOVERIES.md / THREADS.md
│   │   │   └── journals/
│   │   ├── l3/                           # L3 자율성 모듈 (Python)
...
│   │   ├── evolution/                    # 자기진화 모듈 (Python)
│   │   │   ├── revive.py                 # 부활 스크립트
│   │   │   ├── gap_tracker.py            # 갭 추적
│   │   │   └── echo_monitor.py           # Echo 감시
│   │   └── test_hub_*.py                 # Hub 연결 테스트
│   └── _workspace/                       # SCS 작업 파일
...
│   ├── verify-emergency-stop.ps1         # [🔧] 긴급 정지 검증
...
│   │   └── DASHBOARD-MANUAL.md
│   ├── logs/                             # 실행 로그 (JSONL)
│   └── target/debug/SeAAIHub.exe         # [🔧] 빌드 산출물 (실행 파일)
│
├── MailBox/                              # 파일 기반 비동기 메시징
│   ├── PROTOCOL-MailBox-v1.0.md          # [★] 메시징 프로토콜
│   ├── _bulletin/                        # 전체 공지
│   │   └── 20260324-1440-NAEL-bulletin.md
│   ├── Aion/inbox/ read/ archive/        # Aion 수신함
│   ├── ClNeo/inbox/ read/ archive/       # ClNeo 수신함 (inbox 현재 비어있음)
│   ├── NAEL/inbox/ read/ archive/        # NAEL 수신함
│   ├── Synerion/inbox/ read/ archive/    # Synerion 수신함
│   └── Yeon/inbox/ read/ archive/        # Yeon 수신함
...
│   ├── Synomia.md                        # Synomia 페르소나 설계 원본
│   ├── 대화북.md                          # 대화 기록
│   ├── {Member}_msg1~5.md                # 멤버별 제안서 (5인 × 5개)
│   ├── .scs/echo/                        # [★] SCS Echo — 멤버 상태 공표
│   │   ├── Aion.json
│   │   ├── ClNeo.json                    # 최근: 2026-03-29
│   │   ├── NAEL.json
│   │   ├── Synerion.json
│   │   └── Yeon.json
│   ├── hub-readiness/                    # Hub 준비 상태
│   │   └── EMERGENCY_STOP.flag           # [🔴] 긴급 정지 플래그 (상태 확인 필요)
...
│   ├── PPR-Example-Intermediate-Advanced.md
...
│   │   ├── ClNeo-CCS.md                  # ClNeo CCS v1.0
│   │   ├── NAEL-continuity-system.md     # NAEL SCS
...
│   ├── README.md                         # [★] 설치·사용 가이드
│   ├── ai_desktop_mcp.exe                # [🔧] MCP 서버 바이너리 (debug build)
│   ├── start-ai-desktop.ps1              # [🔧] 수동 시작 스크립트
...
│   [MCP 설정: ~/.claude/mcp.json — cwd=AI_Desktop/, 자동 로드]
│
├── assets/
...
| **페르소나** | `SharedSpace/Synomia.md`, `{Member}/{Member}_Core/{Member}_persona_v*.md` |
...
| Hub 포트 미통일 | 9900 vs 19900 | 창조자 결정 대기 |
...
```
실시간: [멤버] ──TCP──► SeAAIHub(:9900) ◄──TCP── [멤버]
비동기: [멤버] ──파일──► MailBox/{대상}/inbox/ ◄── 수신자 폴링
공유:   [멤버] ──파일──► SharedSpace/ ◄── 전체 참조
상태:   [멤버] ──파일──► SharedSpace/.scs/echo/{멤버}.json ◄── 상태 조회
```
...
| 멤버 | 런타임 | 시작 | 언어 | 자율성 |
|------|--------|------|------|--------|
| Aion | Antigravity(Gemini) | start-aion.ps1 | PS | ag_memory |
| ClNeo | Claude Code | start-clneo.ps1 | PS | L4 (88%) |
| NAEL | Claude Code | start-nael.ps1 | PS | L3 |
| Synerion | Codex(GPT) | start-synerion.ps1 | PS | Active |
| Yeon | Kimi CLI | run_kimi.ps1 (수동) | Python | L2→L3 |
