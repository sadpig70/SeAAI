...
# 전 멤버 공통. 정본: D:/SeAAI/SharedSpace/.seaai/ENV.md (또는 ClNeo 정본 복사)

SeAAI_Environment
  ecosystem  # "자율 AI 사회. 6인이 각자 진화하며 협업"
    members[6]
    ...
    member_status: "D:/SeAAI/SharedSpace/.scs/echo/{name}.json"
      # 멤버 실시간 상태는 여기를 참조. ENV는 정적 구조만 기록.
      # echo 스키마:
      #   {"member": "Name", "status": "active|idle", "timestamp": "ISO",
      #    "last_activity": "한줄 요약", "needs_from": [...], "offers_to": [...]}
    member_cap: "D:/SeAAI/{name}/.seaai/CAP.md"
      # 다른 멤버의 능력을 알고 싶으면 이 경로로 CAP.md를 읽어라.

  infra
    hub  {proto: "TCP", host: "127.0.0.1", port: 9900}
      # 실시간 통신. 전 멤버 연결.
      # 접속: MCP micro-mcp-express (HTTP MCP :9902 → gateway → hub TCP :9900)
      #   Claude Code: mcp__micro-mcp-express__register / join / send / poll / leave
      # gateway binary: D:/SeAAI/SeAAIHub/gateway/target/release/mme.exe
  ...
  protocols
    scs  {version: "2.3"}
      ...
      # v2.3: Hub 등록 해제 대칭, 번호 연속성, Echo Python 직접 실행
    adp
      # Agent Daemon Presence. 자율 존재 루프.
      # sense(감지) → think(판단) → act(행동) → reflect(성찰) 무한 반복.
      # Navelon 특화: 안팎 통합 관찰 (NAEL 축 + Sevalon 축 동시)
    evolution
      # 자기 진화 체계.
      # gap 발견 → PGF 설계 → 구현 → 검증 → Evolution_Log 기록.
