...
PERSONA_MD = CORE / "Synerion_persona_v1.md"
...
def runtime_readiness_snapshot() -> dict:
    ...
    shared_bounded_pass = (
        ...
        and set(summary.get("final_members", [])) >= {"Synerion", "ClNeo", "NAEL"}
...
def shared_impact_snapshot(
    ...
    manual_sections = manual or manual_sections_from_project_status()
    reasons: list[str] = []
...
def build_default_manual_sections() -> dict[str, list[str]]:
    return {
        ...
        "NextActions": [
            ...
            "Vera 미검증과 ClNeo/NAEL native entrypoint pending을 닫아 full realtime rollout gating을 줄인다.",
        ],
        "OpenRisks": [
            "room membership 검증 전 direct reply는 계속 차단한다.",
            "native runtime parity는 아직 partial이며 Vera 미검증, ClNeo/NAEL native entrypoint pending이 남아 있다.",
            "현재 머신에서는 Rust Hub TCP가 Winsock 10106으로 막혀 있어 local verification은 file-fallback backend 기준이다.",
        ],
    }


def manual_sections_from_project_status() -> dict[str, list[str]]:
    text = read_text(PROJECT_STATUS)
    defaults = build_default_manual_sections()
    if not text:
        return defaults
    output: dict[str, list[str]] = {}
    for name in MANUAL_SECTION_NAMES:
        items = extract_manual_section(text, name)
        if not items or contains_mojibake(items):
            items = defaults[name]
        output[name] = items
    return output
...
def next_session_recognition_items() -> list[str]:
    return numbered_from_section(
        ...
        default=[
            "AGENTS.md를 읽는다",
            "Synerion core 문서를 읽는다",
            "PROJECT_STATUS와 continuity 파일을 읽는다",
        ],
    )
...
def self_recognition_card_text() -> str:
    ...
    lines = [
        ...
        *(f"{index}. {item}" for index, item in enumerate(next_session_recognition_items(), start=1)),
        ...
        f"- {rel(SYNERION_MD)}",
        f"- {rel(PERSONA_MD)}",
        f"- {rel(CAPABILITIES_MD)}",
        f"- {rel(LIMITS_AUTHORITY_MD)}",
        f"- {rel(PROJECT_STATUS)}",
        f"- {rel(STATE_JSON)}",
    ]
    return "\n".join(lines) + "\n"
...
def self_recognition_payload() -> dict:
    return {
        ...
        "source_docs": [
            rel(SELF_RECOGNITION_CARD_MD),
            rel(CAPABILITIES_MD),
            rel(LIMITS_AUTHORITY_MD),
        ],
    }
...
def self_recognition_core_drift_report() -> dict:
    ...
    if actual_state != expected_state:
        mismatches.append("STATE.json self_recognition block diverges from current docs")
...
def project_status_text() -> str:
    registry = parse_member_registry()
    manual = manual_sections_from_project_status()
    statuses = collect_status_summaries()
    changed = recent_changes(14)
    workspace_assets = latest_files(WORKSPACE, 10)
    readiness = runtime_readiness_snapshot()
    mailbox = mailbox_triage_snapshot()
    shared = shared_impact_snapshot(mailbox=mailbox, readiness=readiness, manual=manual)
    drift_link = drift_evolution_snapshot(readiness=readiness, shared=shared)
    key_docs = [
        "AGENTS.md",
    ...
    def list_block(items: Iterable[str]) -> str:
        return "\n".join(f"- {item}" for item in items)

    def file_block(paths: Iterable[Path]) -> str:
        return "\n".join(
            f"- {rel(path)} ({datetime.fromtimestamp(path.stat().st_mtime).astimezone().strftime('%Y-%m-%d %H:%M:%S %z')})"
            for path in paths
        )

    def raw_block(lines: Iterable[str]) -> str:
        return "\n".join(lines)

    return (
        "# PROJECT_STATUS\n\n"
        f"업데이트 시간: {format_timestamp()}\n"
        f"워크스페이스: {ROOT.as_posix()}\n\n"
        "## 프로젝트 개요\n\n"
        "Synerion은 SeAAI 생태계에서 구조화, 구현, 통합, 검증을 담당하는 동료 에이전트다.\n"
        "이 워크스페이스의 세션 연속성 기준은 PROJECT_STATUS.md이며, 나머지 continuity 파일은 파생 상태로 취급한다.\n\n"
        "## 우선 읽을 문서\n\n"
        f"{list_block(key_docs)}\n\n"
        "## 디렉터리 구조\n\n"
        "- .pgf/\n"
        "- _workspace/\n"
        "- skills/\n"
        ...
        "## 최신 durable 상태\n\n"
        f"{raw_block(statuses or ['- 없음'])}\n\n"
        "## 최근 변경 파일\n\n"
        f"{file_block(changed)}\n\n"
        "## 최근 _workspace 자산\n\n"
        f"{file_block(workspace_assets)}\n\n"
        "## 최신 Hub/ADP 요약\n\n"
        f"{raw_block(build_hub_summary(registry))}\n\n"
        "## Mailbox Advisory\n\n"
        f"{raw_block(mailbox_advisory_lines(mailbox))}\n\n"
        "## Runtime Readiness\n\n"
        f"{raw_block(runtime_readiness_lines(readiness))}\n\n"
        "## Shared Impact\n\n"
        f"{raw_block(shared_impact_lines(shared))}\n\n"
        "## Drift-Evolution Link\n\n"
        f"{raw_block(drift_evolution_lines(drift_link))}\n\n"
        "## 현재 진행 중\n"
        "<!-- MANUAL:ActiveThreads:START -->\n"
        f"{list_block(manual['ActiveThreads'])}\n"
        "<!-- MANUAL:ActiveThreads:END -->\n\n"
        "## 다음 액션\n"
        "<!-- MANUAL:NextActions:START -->\n"
        f"{list_block(manual['NextActions'])}\n"
        "<!-- MANUAL:NextActions:END -->\n\n"
        "## 오픈 리스크\n"
        "<!-- MANUAL:OpenRisks:START -->\n"
        f"{list_block(manual['OpenRisks'])}\n"
        ...
        "## 최신 근거\n\n"
        f"- Evolution: {latest_evolution_heading()}\n"
        f"- Latest report: {latest_report_heading()}\n"
        f"- Member registry updated: {registry.get('updated') or 'unknown'}\n\n"
        "## 재개 체크리스트\n\n"
        "1. AGENTS.md를 읽는다.\n"
...
def echo_payload() -> dict:
    ...
    return {
        ...
        "open_threads": manual["ActiveThreads"],
        "needs_from": {
            "NAEL": "direct reply gating and runtime safety rule verification",
            "ClNeo": "persona-seed / ADP bootstrap alignment review",
        },
        "offers_to": {
            "Aion": "continuity export and echo publication patterns are available",
            "Signalion": "shared roster and readiness alignment support is available",
        },
...
def now_markdown() -> str:
    ...
    lines = [
        ...
        "## 무슨 일이 있었나",
        "",
        f"최근 기준선은 {latest_report_heading()} 이고, 현재 continuity 핵심은 PROJECT_STATUS 중심 복원 체계다.",
...
def resume_summary_text() -> str:
    ...
    lines = [
        ...
        *(f"- {item}" for item in manual["ActiveThreads"]),
        ...
        wal_status_line(),
        f"- NOW snapshot: {now_excerpt()}",
        f"- Self-recognition card: {rel(SELF_RECOGNITION_CARD_MD)}",
...
def self_test_checks() -> list[tuple[str, bool, str]]:
    project_status = read_text(PROJECT_STATUS)
    state = load_json(STATE_JSON, {})
    echo = load_json(ECHO_DIR / "Synerion.json", {})
    readiness = runtime_readiness_snapshot()
    mailbox = mailbox_triage_snapshot()
    checks = [
        ("required_docs", all(path.exists() for path in (PROJECT_STATUS, SESSION_CONTINUITY, SYNERION_MD, PERSONA_MD, OPERATING_CORE_MD, CAPABILITIES_MD, LIMITS_AUTHORITY_MD)), "core continuity documents exist"),
        ("manual_sections", all(extract_manual_section(project_status, name) for name in MANUAL_SECTION_NAMES), "PROJECT_STATUS manual sections are present"),
        ...
        ("self_recognition_card", SELF_RECOGNITION_CARD_MD.exists() and "## Who I Am" in read_text(SELF_RECOGNITION_CARD_MD), "self-recognition card exists"),
        ("capabilities_doc", CAPABILITIES_MD.exists() and "## Capability Summary" in read_text(CAPABILITIES_MD), "capability registry exists"),
        ("limits_doc", LIMITS_AUTHORITY_MD.exists() and "## Hard Limits" in read_text(LIMITS_AUTHORITY_MD), "limits and authority doc exists"),
        ("self_act_lib", SELF_ACT_LIB_MD.exists() and "## Seed Set" in read_text(SELF_ACT_LIB_MD), "self-act library exists"),
        ("adp_bootstrap", ADP_BOOTSTRAP_MD.exists() and "## Persona Seed" in read_text(ADP_BOOTSTRAP_MD), "ADP bootstrap includes persona seed"),
        ("echo_json", bool(echo and echo.get("member") == "Synerion"), "Shared Echo JSON parses and identifies Synerion"),
        ("runtime_adaptation", RUNTIME_ADAPTATION_MD.exists() and "## 환경 감지" in read_text(RUNTIME_ADAPTATION_MD), "runtime adaptation guide exists"),
...
def sync_continuity_files() -> None:
    ...
    try:
        mailbox = mailbox_triage_snapshot()
        readiness = runtime_readiness_snapshot()
        write_text(PROJECT_STATUS, project_status_text())
        ...
        if success:
            clear_wal()
    if success:
        # The card is a derived snapshot; write it after WAL is cleared so reopen output sees a clean state.
